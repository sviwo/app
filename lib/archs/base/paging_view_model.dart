...
abstract class PagingViewModel<T> extends BaseViewModel {
  ...
  /// 请求并加载分页数据
  Future<ResData<ResList<T>>?> loadPaging(
      Future<ResData<ResList<T>>> api, bool isRefresh,
      {bool showLoading = false, String? errorMsg}) async {
    if (isRefresh) {
      _pageNum = 1;
    } else if (listData.length >= totalSize) {
      return null;
    }

    try {
      enablePullDown = false;
      var res = await loadApiData(api,
          showLoading: showLoading,
          errorMsg: errorMsg,
          handlePageState: false, onFailed: (newErrorMsg) {
      ...
      if (res.data == null) {
        updatePageState(PageState.error,
            stateMsg: errorMsg ?? res.msg ?? '未获取到有效数据');
        return null;
      }
      var data = res.data!;
      enablePullDown = true;
      ...
      enablePullUp = listData.length < totalSize;
      if (enablePullUp) {
        _pageNum++;
      }
